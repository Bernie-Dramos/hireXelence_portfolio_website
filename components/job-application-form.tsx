  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Button
        ...
        className="w-full group relative bg-gradient-to-r from-emerald-primary to-emerald-light hover:shadow-emerald-primary/50 text-white py-6 text-lg font-semibold shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden mt-6"
      >
        {isSubmitting && (
        ...
        <span className={`relative flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`}>
          {isSubmitting ? 'Submitting Application...' : 'Submit Application'}
          {!isSubmitting && <ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform duration-500" />}
        </span>
      </Button>
    </form>
  )
